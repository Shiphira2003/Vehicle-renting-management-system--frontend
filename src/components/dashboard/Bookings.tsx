...
import { useGetAllBookingsForOneUserByIdQuery } from '../../features/api/bookingsApi';
...
const UserBookings = () => {
  ...
  const { data: bookings, isLoading, isError, error } = useGetAllBookingsForOneUserByIdQuery(userId!, {
  ...
  if (isError) {
    Swal.fire({
      icon: 'error',
      title: 'Booking Error',
      text: `Failed to fetch bookings: ${(error)}`,
    });
